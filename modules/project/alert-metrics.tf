...
resource "google_monitoring_notification_channel" "default" {
  for_each     = { for k, v in local.logging_metrics_alerts : k => v if v.notification_channels == [] }
  project      = "jetstack-joshua-wright"
  display_name = "Default Email Notification"
  type         = "email"
  labels = {
    email_address = "joshua.wright@jetstack.io"
  }
}
...
resource "google_monitoring_alert_policy" "default" {
  ...
  conditions {
    ...
    condition_threshold {
      ...
      filter     = "resource.type = \"global\" AND metric.type = \"logging.googleapis.com/user/${google_logging_metric.default[each.value.name].name}\""
      trigger {
  ...
  notification_channels = [
    try(google_monitoring_notification_channel.default[each.value.name].id,
    split(",", join(",", each.value.notification_channels)))
  ]
  alert_strategy {
    auto_close = "604800s"
  }
